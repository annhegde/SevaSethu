...
class SevaSethuSupabaseUser extends BaseAuthUser {
  SevaSethuSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> sevaSethuSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SevaSethuSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
